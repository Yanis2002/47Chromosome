...
function blackhole(element) {
    ...
    class Star {
        constructor() {
            ...
            this.speed = (Math.floor(Math.random() * 1.0) + 0.5) * Math.PI / 180; // The rate at which this star will orbit (reduced rotation speed)
            ...
            // Color with neon accents similar to site theme
            const opacity = (1 - ((this.orbital) / 255));
            const useNeon = Math.random() > 0.7; // 30% chance for neon color
            if (useNeon) {
                const neonColors = [
                ...
                this.color = 'rgba(255,255,255,' + opacity + ')'; // white
            }
    ...
    centerHover.addEventListener('click', function() {
        ...
        setTimeout(() => {
            // Path from interactive-blackhole/dist/ to docs/index.html
    ...
    function loop() {
        ...
        context.fillStyle = 'rgba(10,10,10,0.2)'; // somewhat clear the context, this way there will be trails behind the stars (darker background)
    ...
    function init() {
        context.fillStyle = 'rgba(10,10,10,1)'; // Initial clear of the canvas (darker background)
        context.fillRect(0, 0, cw, ch);
